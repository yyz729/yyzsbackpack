...
//        context.blitSprite(RenderPipelines.GUI_TEXTURED, ResourceLocation.withDefaultNamespace("social_interactions/background"), left, top, 0, 0, width, height, width, height);
    ...
    public static void renderBackpackPreview(GuiGraphics guiGraphics, Minecraft minecraft, AbstractContainerMenu menu, @Nullable Slot hoveredSlot, int leftPos, int topPos, int imageWidth, int imageHeight){
        ...
        switch (Backpack.getConfig().tooltip_modifier.toLowerCase()) {
            case "shift" -> requireKey = InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 340) || InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 344);
            case "alt" -> requireKey = InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 342) || InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 346);
            case "ctrl" -> requireKey = InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 341) || InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 345);
            case "none" -> requireKey = true; // 不需要按键
            default -> {
                // 无效配置时使用默认值（shift）
                if (!(InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 340) || InputConstants.isKeyDown(Minecraft.getInstance().getWindow(), 344))) return;
                return;
            }
        }
