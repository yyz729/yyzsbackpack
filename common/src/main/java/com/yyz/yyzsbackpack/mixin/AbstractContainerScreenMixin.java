...
public abstract class AbstractContainerScreenMixin<T extends AbstractContainerMenu> extends Screen implements BackpackScreen {
    ...
    @Shadow protected abstract boolean hasClickedOutside(double d, double e, int i, int j);
    ...
    @Redirect(method = "mouseClicked", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/screens/inventory/AbstractContainerScreen;hasClickedOutside(DDII)Z"))
    private boolean handleMouseClicked(AbstractContainerScreen<?> instance, double mouseX, double mouseY, int left, int top) {
        return menu instanceof BackpackMenu ? BackpackHelper.isClickOutsideExtendedBounds(inventory, hasClickedOutside(mouseX, mouseY, leftPos, topPos), mouseX, mouseY, leftPos, topPos, imageWidth, imageHeight, shouldRenderBackpack, (BackpackMenu) this.menu ) : hasClickedOutside(mouseX, mouseY, leftPos, topPos);
    }

    @Redirect(method = "mouseReleased", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/screens/inventory/AbstractContainerScreen;hasClickedOutside(DDII)Z"))
...
//        }
    ...
    private void onKeyPressed(MouseButtonEvent mouseButtonEvent, boolean bl, CallbackInfoReturnable<Boolean> cir) {
        if (mouseButtonEvent.hasShiftDown() && menu.getCarried().isEmpty()) {
            shiftPressed = true;
            type = mouseButtonEvent.button();
        }
    }
