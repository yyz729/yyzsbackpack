...
public class BackpackRenderer {
    ...
    public static void renderEquipSlotBackground(InventoryMenu menu, GuiGraphics guiGraphics, int x, int y){
        ...
        guiGraphics.blit(RenderType::guiTextured, ResourceLocation.fromNamespaceAndPath(Backpack.MOD_ID, "textures/gui/slot.png"),  x + ((BackpackMenu)menu).getBackpackEquipSlotX(),  y+ ((BackpackMenu)menu).getBackpackEquipSlotY(), 0, 0, 18, 18, 18, 18);
...
//        context.blitSprite(RenderPipelines.GUI_TEXTURED, ResourceLocation.withDefaultNamespace("social_interactions/background"), left, top, 0, 0, width, height, width, height);
        context.blit(RenderType::guiTextured, texture, left, top, 0, 0, width, height, width, height);
    }
