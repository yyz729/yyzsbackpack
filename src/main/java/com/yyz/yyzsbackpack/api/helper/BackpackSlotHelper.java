...
public final class BackpackSlotHelper {
    ...
    private static void ensureBackpackId(ItemStack stack) {
        ...
        if (!tag.contains(BACKPACK_ID_KEY)) {
            tag.putString(BACKPACK_ID_KEY, UUID.randomUUID().toString());
        }
    }
    ...
    private static String getBackpackId(ItemStack stack) {
        ...
        if (tag != null && tag.contains(BACKPACK_ID_KEY)) {
            return tag.getString(BACKPACK_ID_KEY);
        }
        return null;
    }

    private static void setBackpackId(ItemStack stack, UUID uuid) {
        if (stack.isEmpty() || !(stack.getItem() instanceof BackpackItem)) return;
        CompoundTag tag = stack.getOrCreateTag();
        tag.putString(BACKPACK_ID_KEY, uuid.toString());
    }
    // ==================================
    ...
    public static ItemStack getSelectedBackpack(Player player) {
        ...
        String currentUUID = getBackpackId(currentStack);
    ...
    public static void setSelectedIndex(Player player, int index) {
        ...
        if (index >= 0 && index < slots.size()) {
            ...
            String id = getBackpackId(stack);
            if (id != null) {
                ((IBackpackData) player).yyzsbackpack$setSelectedBackpackUuid(id);
                return;
            }
        }
        ((IBackpackData) player).yyzsbackpack$setSelectedBackpackUuid(null);
    ...
    public static void updateWeightEffect(Player player) {
        ...
        boolean hasEffect = player.hasEffect(ModEffects.HEAVY.get());

        if (backpackCount > Backpack.getConfig().heavy) {
            if (!hasEffect) {
                player.addEffect(new MobEffectInstance(
                        ModEffects.HEAVY.get(),
