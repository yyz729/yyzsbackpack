...
//	@Redirect(method = "getContainerSize", at = @At(value = "INVOKE", target = "Lnet/minecraft/core/NonNullList;size()I"))
//	private int modifyContainerSize(NonNullList<ItemStack> instance) {
//		// 检查是否有背包物品
//		ItemStack backpackStack = BackpackHelper.getEquipped(player);
//		if (backpackStack.getItem() instanceof BackpackItem backpackItem) {
//			// 基础槽位数 + 背包列数 * 9
//			return 36 + backpackItem.getBackpackType().getColumns() * 9 + this.armor.size() + this.offhand.size();
//		}
//		return 36 + this.armor.size() + this.offhand.size(); // 没有背包时返回基础槽位数
//	}
